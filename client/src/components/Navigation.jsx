...
function NavBar() {
    ...
    return <nav className={`py-[35px] px-[60px] sm:px-[20px] sm:py-0 w-full bg-base ${!hideMenu ? 'sm:fixed sm:left-0 sm:z-50 sm:h-screen' : null} flex items-center justify-between gap-[97px] sm:gap-[0px]`}>
        ...
        <div className={`fixed hidden top-[10vh] left-0 h-screen ${hideMenu ? 'translate-x-[-100%]' : 'translate-x-0'} sm:flex flex-col bg-white px-5 py-5 sm:h-full sm:w-[100vw] transform transition-transform ease-in-out duration-300 z-40 lg:w-full`}>
            ...
            <div className='flex flex-col items-center gap-[8px] justify-between text-[16px] font-ui-semi'>
                <Link to={'/login'} className='w-full' onClick={handleHideModal}>
                ...
                <Link to={'/signup'} className='w-full' onClick={handleHideModal}>
...
function toggleNavBar(path) {
    if (path === '/signup' || path === '/forgotPassword') return null
    return <NavBar />
}
